const MPCSignature = require('../src/utils/mpc-signature');
const { ShamirSecretSharing } = require('../../mpc-core/src/index');
const EC = require('elliptic').ec;
const keccak = require('keccak');

// 初始化
const mpcSignature = new MPCSignature();
...
try {
  const completeSignature = mpcSignature.combineSignatures(
    partialSignatures,
    shares.slice(0, 2),
    messageHash
  );
...
try {
  // 测试不足的份额
  const insufficientSignatures = mpcSignature.combineSignatures(
    [partialSignatures[0]], // 只有一个签名
    [shares[0]],
    messageHash
  );
} catch (error) {
  console.log(`   不足份额测试: ${error.message}`);
}
